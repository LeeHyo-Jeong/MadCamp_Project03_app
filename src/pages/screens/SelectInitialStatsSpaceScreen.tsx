...
type SelectInitialStatsScreenSpaceNavigationProp = StackNavigationProp<
  RootStackParamList,
  'SelectInitialStatsSpace'
>;

type SelectInitialStatsSpaceScreenRouteProp = RouteProp<
  RootStackParamList,
  'SelectInitialStatsSpace'
>;

const stats = ['공격력', '방어력', '체력', '이동속도', '지혜', '운'];

type Stats = {[key in (typeof stats)[number]]: number};

const SelectInitialStatsSpaceScreen: React.FC = () => {
  const navigation =
    useNavigation<SelectInitialStatsScreenSpaceNavigationProp>();
  const route = useRoute<SelectInitialStatsSpaceScreenRouteProp>();
...
export default SelectInitialStatsSpaceScreen;
