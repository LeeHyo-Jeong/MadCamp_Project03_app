...
import {RootStackParamList, Stats} from '../../types/NavigationTypes';
import axios from 'axios';
import {API_URL} from '@env';
import AsyncStorage from '@react-native-async-storage/async-storage';

type SelectInitialStatsFutureScreenNavigationProp = StackNavigationProp<
  RootStackParamList,
  'SelectInitialStatsFuture'
>;

type SelectInitialStatsFutureScreenRouteProp = RouteProp<
  RootStackParamList,
  'SelectInitialStatsFuture'
>;
...
const SelectInitialStatsFutureScreen: React.FC = () => {
  const navigation =
    useNavigation<SelectInitialStatsFutureScreenNavigationProp>();
  const route = useRoute<SelectInitialStatsFutureScreenRouteProp>();
  ...
  const handleConfirm = async () => {
    ...
    // dummy room Id
    const roomId = '1234';
    navigation.navigate('GamePlay', {roomId});
    */
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // AsyncStorage에 roomId를 저장
        await AsyncStorage.setItem(`currntRoomId`, roomId);
...
export default SelectInitialStatsFutureScreen;
